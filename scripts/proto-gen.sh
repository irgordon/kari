...
PROTO_GO_PACKAGE="$(awk -F'"' '/^option go_package = / {print $2}' "$PROTO_SRC" | sed 's/;.*$//')"

if [[ -z "$PROTO_GO_PACKAGE" ]]; then
  echo "❌ go_package option was not found in $PROTO_SRC"
  exit 1
fi

if [[ "$PROTO_GO_PACKAGE" != "$EXPECTED_GO_PACKAGE" ]]; then
  echo "❌ go_package mismatch detected"
  echo "   module path : $MODULE_PATH"
  echo "   expected    : ${EXPECTED_GO_PACKAGE};rustagent"
  echo "   actual      : $PROTO_GO_PACKAGE"
  echo "   Fix proto/kari/agent/v1/agent.proto before regeneration."
  exit 1
fi
...
# `go_package = <module>/api/internal/grpc/rustagent;rustagent` + `module=<module>`
